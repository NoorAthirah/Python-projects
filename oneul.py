...
def get_events(): #to import library
   list_events = []
...
def days_between_dates(date1, date2): #o calculate the difference between event dates and today's date
   time_between = str(date1 - date2)
   number_of_days = time_between.split(' ')
   return number_of_days[0]

def update_clock(): #to update the digital clock live by displaying the current time and date at a 1-second interval.
   now = datetime.now()
