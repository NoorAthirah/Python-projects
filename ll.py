...
# to show an input window consisting of a label, entry, and button!
def custom_askstring(title, prompt, dialog_size="450x150", bg_color="lightblue", font_size=12, font_weight="bold"):
   ...
   # add an 'OK' button so that the user can submit the entered data
#    def ok():
...
#    button.pack()
#    dialog.geometry(dialog_size)  # Setting Dialog Size
...
# a while loop so that our windows continue to be displayed
while True:
   query_country_input = custom_askstring('Country', 'Type the name of a country:', dialog_size="450x150", bg_color="lightgreen", font_size=14, font_weight="bold")
...
#        else  :
#            new_city = custom_askstring('Teach me', 'I don\'t know! What is the capital city of ' + query_country + '?', dialog_size="450x150", bg_color="lightblue", font_size=12, font_weight="bold")
